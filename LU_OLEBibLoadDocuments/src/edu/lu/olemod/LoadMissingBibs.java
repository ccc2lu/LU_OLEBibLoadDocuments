...
public class LoadMissingBibs {
	...
	public static void main(String args[]) {
		...
		String lehighDataDir = args[3];
		boolean dostage = args[5].equals("Y");
		...
		try {
			...
			LU_BuildInstance instanceBuilder = new LU_BuildInstance(dostage, dumpdir + "/mod.allcallnums.txt", 
					...
					dumpdir + "/mod.allitems.txt",
					lehighDataDir + "/Lehigh Locations.csv",
					lehighDataDir + "/sfx_export_portfolios.csv", ole_em, migration_em);
			...
			try {
				...
				String line, key;
				String parts[];
				inFile = new BufferedReader(new FileReader(args[4]));
			...
			try {
				...
				//MarcReader reader = new MarcXmlReader(inputsource);

				InputStream defaultkeysInput = new FileInputStream(args[2]);
	...
	public static Bib buildBib(Record record, Record defaultkeysrecord) {
		//Log(System.err, "Formatting key: !" + record.getControlNumber() + "!");
		...
		//Log(System.err, "Formatted to: !" + catkey + "!");
